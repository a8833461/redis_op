 ...
 */
/* -------------------------------------------*/
int rop_get_string(redisContext *conn, char *key,RVALUES value)
{
    int retn = 0;
    redisReply *reply = NULL;
    reply = redisCommand(conn, "get %s",key);
    LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"get %s\n",key);
    if ( reply->str )
    {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"reply->str not null\n");
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"reply->str is %s\n",reply->str);
        strcpy(value,reply->str);
    }
    else
    {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC,"reply->str is null\n");
        retn = -1;
    }
	freeReplyObject(reply);
    return retn;
